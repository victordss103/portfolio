...
const ThemeSwitcher = () => {
  ...
  useEffect(() => {
    // On met à jour le thème après le montage du composant
    if (theme === "system") {
      setCurrentTheme(systemTheme);  // Le thème système (light/dark)
    } else {
      setCurrentTheme(theme);  // Le thème défini par l'utilisateur
    }
  }, [theme, systemTheme]);

  // Si `currentTheme` n'est pas encore défini, on ne rend rien (attente du client)
